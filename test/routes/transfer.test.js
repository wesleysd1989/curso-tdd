...
describe('Ao alterar uma tranferência valida ...', () => {
    let transferId;
    let income;
    let outcome
    ...
    test('Deve retornar o status 200 e os dados da transferência', () => {
        ...
        .set('authorization', `bearer ${TOKEN}`)
        .send({ description: 'Transfer Updated', user_id: 100000, acc_ori_id: 100000, acc_dest_id: 100001, ammount: 500, date: new Date() })
        .then(async (res) => {
            expect(res.status).toBe(200);
